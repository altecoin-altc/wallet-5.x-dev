...
int64_t CBlockIndex::MinPastBlockTime() const
{
    ...
    // so we allow the nBlockTimeProtocolV2 (Deviant v4.0) to be at most (180-14) seconds earlier than previous block
...
{
    if (vStakeModifier.empty() || Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V5_0))
        return 0;
