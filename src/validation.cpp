...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransactionRef& _tx, bool fLimitFree, bool* pfMissingInputs, int64_t nAcceptTime, bool fOverrideMempoolLimit, bool fRejectAbsurdFee, bool ignoreFees, std::vector<COutPoint>& coins_to_uncache)
{
    AssertLockHeld(cs_main);
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && hasTxZerocoins)
        return state.DoS(10, error("%s : Zerocoin transactions are temporarily disabled for maintenance", __func__), REJECT_INVALID, "bad-tx-zerocoin-maintenance");
    if (sporkManager.IsSporkActive(SPORK_20_SAPLING_MAINTENANCE) && tx.IsShieldedTx())
        return state.DoS(10, error("%s : Shielded transactions are temporarily disabled for maintenance", __func__), REJECT_INVALID, "bad-tx-sapling-maintenance");
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight > 0 && nHeight < 501) {
        ...
        nSubsidy = 13.448 * COIN;
    } else if (nHeight < 1802600 && nHeight >= 1276950) {
        nSubsidy = 11.027 * COIN;
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
            ...
            if (fAddressIndex || fSpentIndex) {
                for (size_t j = 0; j < tx.vin.size(); j++) {
                    ...
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
                        ...
                        hashBytes = uint160(std::vector<unsigned char>(prevout.scriptPubKey.begin() + 2, prevout.scriptPubKey.end()));
                        addressType = 1;
                    } else {
                        hashBytes.SetNull();
                        addressType = 0;
    ...
    if (!IsBlockValueValid(pindex->nHeight, nExpectedMint, nMint)) {
        return state.DoS(100, error("ConnectBlock() : reward pays too much (actual=%s vs limit=%s)", FormatMoney(nMint), FormatMoney(nExpectedMint)),
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    g_IsV6Active = Params().GetConsensus().NetworkUpgradeActive(pindexNew->nHeight, Consensus::UPGRADE_V5_0);
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (!Params().IsRegTestNet() && block.IsProofOfWork() && (pindexPrev->nHeight + 1 <= 201)) {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        if (!CheckProofOfStake(block, strError, pindexPrev))
            if (Params().GetConsensus().NetworkUpgradeActive(pindexPrev->nHeight, Consensus::UPGRADE_V5_0)) {
            ...
            if (hasZDEVInputs) {
                for (const CTxIn& zdevInput : zDEVInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zdev");
                }
            }
        }
            ...
            if (!isBlockFromFork)
                for (const CTxIn& zdevInput : zDEVInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpend(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100, error("%s: main chain ContextualCheckZerocoinSpend failed for tx %s", __func__, stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zdev");
                }
        }
    }
...
int ActiveProtocol()
{
    // SPORK_14 was used for 80005 (v3.0.0), commented out now.
    //if (sporkManager.IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
    //    return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 is used for 80004 (v2.1.0)
